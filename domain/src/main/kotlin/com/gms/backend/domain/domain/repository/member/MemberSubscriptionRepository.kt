...
interface MemberSubscriptionRepository : JpaRepository<MemberSubscription, UUID> {
    @Query(
        value = $$"""
        SELECT new com.gms.backend.domain.application.rest.member.MemberSubscriptionController$MemberSubscriptionTableDTO(
        ms.id,
        ms.actorId,
        m.firstName,
        m.surname,
        m.middleName,
        ms.subscriptionAvailedId,
        ...
        JOIN ms.subscriptionAvailed s
        JOIN ms.branch b
        LEFT JOIN Member m on ms.actor = m.actor
        ORDER BY ms.updatedAt DESC
        """,
        countQuery = "SELECT COUNT(m) FROM MemberSubscription m"
    )
    ...
    fun findByMemberId(id: UUID): Optional<MemberSubscriptionController.MemberSubscriptionTableDTO>

    fun findAllByActorIdAndStatus(
        memberId: UUID, status: MemberSubscription.MemberSubscriptionStatus
    ...
    fun findIdsByActorIdAndStatus(
        @Param("actorId") actorId: UUID, @Param("status") status: MemberSubscription.MemberSubscriptionStatus
    ): List<UUID>
